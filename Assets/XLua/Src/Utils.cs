...
namespace XLua
{
    ...
    public static partial class Utils
    {
        ...
        public static void loadUpvalue(RealStatePtr L, Type type, string metafunc, int index)
        {
            ...
            // 用 "LuaIndexs"、"LuaNewIndexs"、"LuaClassIndexs"、"LuaClassNewIndexs"从G[LUA_REGISTRYINDEX]获取表
            LuaAPI.lua_rawget(L, LuaIndexes.LUA_REGISTRYINDEX);
            // -1 type的udata
            translator.Push(L, type);
            // 获取G[LUA_REGISTRYINDEX]["LuaIndexs"][udata]，pop udata，这里-1得到的是cclosure obj_indexer
            LuaAPI.lua_rawget(L, -2);
            // pop G[LUA_REGISTRYINDEX]["LuaIndexs"]
            LuaAPI.lua_remove(L, -2);
            for (int i = 1; i <= index; i++)
            {
                // 获取obj_indexer的upvalue，
        ...
        /// -3 保存method的table
        /// -4 MT
        /// -5 要测试一下 TODO 当前类型的一个table
        /// 开始类型注册
        /// </summary>
        /// <param name="type"></param>
        /// <param name="L"></param>
        /// <param name="translator"></param>
        /// <param name="meta_count">没用</param>
        /// <param name="method_count">warp中需要注入的函数数量</param>
        /// <param name="getter_count">getter函数数量</param>
        /// <param name="setter_count">setter函数数量</param>
        ...
        {
            // 后面几个函数的数量，应该是生成warp代码时算出来的，不同类型的函数idx也不同，可以参考UnityEngineVector2Wrap这个文件
            if (type == null)
            {
            ...
            else
            {
                ...
                // TryDelayWrapLoader中第一次创建类型是创建了
                LuaAPI.luaL_getmetatable(L, type.FullName);
                //  如果没有找到pop nil, 创建一个新的matetable: 下面用MT代替这个{}
                if (LuaAPI.lua_isnil(L, -1))
                {
                    LuaAPI.lua_pop(L, 1);
                    LuaAPI.luaL_newmetatable(L, type.FullName);
            ...
            // static int tag = 0; xlua_tag的返回值是&tag
            LuaAPI.lua_pushlightuserdata(L, LuaAPI.xlua_tag());
            LuaAPI.lua_pushnumber(L, 1);
            // MT[0] = 1，执行后pop 0和1
            LuaAPI.lua_rawset(L, -3);

            // translator.HasCustomOp(type) = FALSE，所以这个condition最后是TRUE
            // HasCustomOp背后是custom_push_funcs，它的赋值在IniterAdderUnityEngineVector2:Init
            // 从生成的类型来看，应该是把enum和structure做了特别的处理
            if ((type == null || !translator.HasCustomOp(type)) && type != typeof(decimal))
            {
                // MT.__gc = GcMeta = StaticLuaCallbacks.LuaGC
                LuaAPI.xlua_pushasciistring(L, "__gc");
                LuaAPI.lua_pushstdcallcfunction(L, translator.metaFunctions.GcMeta);
                LuaAPI.lua_rawset(L, -3);
            }

            // MT.__tostring = ToStringMeta = StaticLuaCallbacks.ToString
            LuaAPI.xlua_pushasciistring(L, "__tostring");
            LuaAPI.lua_pushstdcallcfunction(L, translator.metaFunctions.ToStringMeta);
            LuaAPI.lua_rawset(L, -3);

            // 极限情况下，会压栈3个nil。不管如何都会有3个对象压栈
            if (method_count == 0)
            {
                // 宏展开：(&(L->top)->val)->tt_ = LUA_VNIL = ((0) | ((0) << 4)) = 0
                LuaAPI.lua_pushnil(L);
            }
            else
            {
                // 创建一个table，hash部分size是method_count
            ...
            {
                // 创建一个table，hash部分size是getter_count
            ...
            {
                // 创建一个table，hash部分size是setter_count
        ...
        /// <summary>
        /// idx绝对值
        /// 以UnityEngineGameObjectWrap为例，top=5, idx= -3 => 5 -3 +1 = 3
        /// </summary>
...
#else
        /// <summary>
        /// 组装methodtable、setter table、getter table。
        /// 结束后栈上只有类table，BeginObjectRegister中创建的四个table都被pop
        /// </summary>
        /// <param name="type"></param>
        /// <param name="L"></param>
        /// <param name="translator"></param>
...
#endif
        {
            /*
             * 干了个啥：
             *  _G[LUA_REGISTRYINDEX][LuaIndexs][type udata] = cclosure obj_indexer
             * mt._index = cclosure obj_indexer (METHOD table + GETTER FUNCTION table + etc)
             *  
             * _G[LUA_REGISTRYINDEX]["LuaNewIndexs"][type udata] = cclosure obj_newindexer
             * mt._newindex = cclosure obj_newindexer (SETTER FUNCTION TABLE + etc)
             */
            // 获取BeginObjectRegister中创建的那些table
            ...
            //begin index gen
            // 6 
            // 因为栈顶在变化，所以采用从下向上的idx，妙
            LuaAPI.xlua_pushasciistring(L, "__index");
            // 7 copy METHOD table
            LuaAPI.lua_pushvalue(L, method_idx);
            // 8 copy GETTER FUNCTION table
            ...
            // gen部分的代码，type一定是有的
            // type udata， 也是objects的index  10
            translator.Push(L, type == null ? base_type : type.BaseType());
            // "LuaIndexs" 11
            LuaAPI.xlua_pushasciistring(L, LuaIndexsFieldName);
            // 拿到table _G[LUA_REGISTRYINDEX][LuaIndexs]
            LuaAPI.lua_rawget(L, LuaIndexes.LUA_REGISTRYINDEX);
...
#else
                // cclosure arrayIndexer
                LuaAPI.lua_pushstdcallcfunction(L, arrayIndexer);
#endif
            }

            // 到此为止栈数据
            // 13  nil                                -- gen_obj_indexer里push的
            // 12  cclosure arrayIndexer
            // 11  _G[LUA_REGISTRYINDEX][LuaIndexs]   -- lua_rawget时pop "LuaIndexs"
            // 10  type udata
            // 9   cclosure csIndexer
            // 8   GETTER FUNCTION TABLE
            // 7   METHOD TABLE
            // 6   "__index"
            // 5   SETTER FUNCTION TABLE
            // 4   GETTER FUNCTION TABLE
            // 3   METHOD TABLE
            // 2   METATABLE
            // 1  "UnityEngine.GameObject" 
            // obj_indexer: [1]: methods, [2]:getters, [3]:csindexer, [4]:base, [5]:indexfuncs, [6]:arrayindexer, [7]:baseindex
            // 创建cclosure obj_indexer，top-7刚好是 METHOD table。对应obj_indexer的7个upvalue
            LuaAPI.gen_obj_indexer(L);
            // 执行完成后的stack
            // 7   cclosure obj_indexer
            // 6   "__index"
            // 5   SETTER FUNCTION TABLE
            // 4   GETTER FUNCTION TABLE
            // 3   METHOD TABLE
            ...
            {
                LuaAPI.xlua_pushasciistring(L, LuaIndexsFieldName);
                // get  _G[LUA_REGISTRYINDEX][LuaIndexs]
                LuaAPI.lua_rawget(L, LuaIndexes.LUA_REGISTRYINDEX);//store in lua indexs function tables
                translator.Push(L, type);
                LuaAPI.lua_pushvalue(L, -3);
                // _G[LUA_REGISTRYINDEX][LuaIndexs][type udata] = cclosure obj_indexer
                // 这里很核心啊，等于每一个gen类和它所有的方法都被存起来了。但是这个是存在Lua里的
                LuaAPI.lua_rawset(L, -3);
                // pop _G[LUA_REGISTRYINDEX][LuaIndexs]
                LuaAPI.lua_pop(L, 1);
            }
            // 执行完成后的stack没有变化
            // 7   cclosure obj_indexer
            ...
            // METATABLE._index = cclosure obj_indexer
            // 所以最后去找对象都是走的cclosure obj_indexer对象
            ...
            // 执行完成后的stack
            // 12  nil                             -- gen_obj_newindexer push nil
            // 11  cclosure arrayNewIndexer
            // 10   _G[LUA_REGISTRYINDEX]["LuaNewIndexs"]
            // 9   type udata
            // 8   cclosure csNewIndexer
            // 7   SETTER FUNCTION TABLE
            // 6   "__newindex"
            // 5   SETTER FUNCTION TABLE
            // 4   GETTER FUNCTION TABLE
            // 3   METHOD TABLE
            ...
            // obj_newindexer: [1]:setters, [2]:csnewindexer, [3]:base, [4]:newindexfuncs, [5]:arrayindexer, [6]:basenewindex
            // 创建cclosure obj_newindexer，top-7刚好是 METHOD table。对应obj_indexer的7个upvalue
            LuaAPI.gen_obj_newindexer(L);

            // 执行完成后的stack
            // 7   cclosure obj_newindexer
            // 6   "__newindex"
            // 5   SETTER FUNCTION TABLE
            // 4   GETTER FUNCTION TABLE
            // 3   METHOD TABLE
            ...
            if (type != null)
            {
                ...
                // _G[LUA_REGISTRYINDEX]["LuaNewIndexs"][type udata] = cclosure obj_newindexer
                LuaAPI.lua_rawset(L, -3);
                // pop _G[LUA_REGISTRYINDEX]["LuaNewIndexs"]
                LuaAPI.lua_pop(L, 1);
            }
            ...
            // METATABLE.__newindex = cclosure obj_newindexer
            // 所以最后去创建新的表字段都是走的cclosure obj_newindexer对象
            LuaAPI.lua_rawset(L, meta_idx);
            // 执行完成后的stack
            // 5   SETTER FUNCTION TABLE
            // 4   GETTER FUNCTION TABLE
        ...
        {
            // 以UnityEngineGameObjectWrap的GetComponent为例，METHOD_IDX = -3
            // lua_gettop: 获取栈中的元素个数 = 5，idx = -3， 所以换算后idx = 3。
            // 这就很符合BeginObjectRegister的结果，method放在从下向上的第三个位置
            // 同理，GETTER_IDX = -2， idx换算后是4。SETTER_IDX = -1换算后是5。
            idx = abs_idx(LuaAPI.lua_gettop(L), idx);
            LuaAPI.xlua_pushasciistring(L, name);
            LuaAPI.lua_pushstdcallcfunction(L, func);
            // method[GetComponent] = _m_GetComponent
            LuaAPI.lua_rawset(L, idx);
        }
#endif
        ...
        /// Class的注入
        /// </summary>
        /// <param name="type">值就是System.RuntimeType</param>
        /// <param name="L"></param>
        /// <param name="creator">构造器函数</param>
        /// <param name="class_field_count">字段数量</param>
        /// <param name="static_getter_count">getter数量</param>
        /// <param name="static_setter_count">setter数量</param>
        public static void BeginClassRegister(Type type, RealStatePtr L, LuaCSFunction creator, int class_field_count,
            int static_getter_count, int static_setter_count)
#endif
        {
            ObjectTranslator translator = ObjectTranslatorPool.Instance.Find(L);
            // 创建字段表，下文叫 class_field_t，
            LuaAPI.lua_createtable(L, 0, class_field_count);
            // test
            LuaAPI.xlua_pushasciistring(L, "tsai");
            LuaAPI.xlua_pushasciistring(L, type.FullName);
            LuaAPI.lua_rawset(L, -3);
            // end
            LuaAPI.xlua_pushasciistring(L, "UnderlyingSystemType");
            translator.PushAny(L, type);
            // class_field_t[UnderlyingSystemType] = type
            LuaAPI.lua_rawset(L, -3);
            // gettop = 2， 1："UnityEngine.GameObject" 2：class_field_t
...
#endif
                // metatabel.__call = cclosure creator
                LuaAPI.lua_rawset(L, -3);
            }
            // 静态字段产生的get.set方法存储的表 
            ...
            // setmetatable(class_field_t, metatabel)
            LuaAPI.lua_setmetatable(L, cls_table);

            int top = LuaAPI.lua_gettop(L);

            //PrintStack(L, type, typeof(UnityEngine.GameObject));
            // -1 setter table
            // -2 getter table
            // -3 metatable
            // -4 class_field_t
            // -5："UnityEngine.GameObject" 
        }
        ...
        public static void EndClassRegister(Type type, RealStatePtr L, ObjectTranslator translator)
        {
            ...
            // 到此为止栈数据
            // 11  nil                                -- gen_cls_indexer里push的
            // 10  _G[LUA_REGISTRYINDEX]["LuaClassIndexs"]   -- lua_rawget时pop "LuaClassIndexs"
            // 9 type udata
            // 8 class_field_t
            // 7 getter table
            // 6 __index
            ...
            // _G[LUA_REGISTRYINDEX]["LuaClassIndexs"]
            LuaAPI.lua_rawget(L, LuaIndexes.LUA_REGISTRYINDEX);//store in lua indexs function tables
            translator.Push(L, type);
            LuaAPI.lua_pushvalue(L, -3);
            // _G[LUA_REGISTRYINDEX]["LuaClassIndexs"][type udata] = cclosure cls_indexer
            LuaAPI.lua_rawset(L, -3);
            // pop _G[LUA_REGISTRYINDEX]["LuaClassIndexs"]
            LuaAPI.lua_pop(L, 1);
            ...
            // 目前为止stack
            // nil                     -- gen_cls_newindexer push
            // 9  _G[LUA_REGISTRYINDEX]["LuaClassNewIndexs"]
            // 8 type udata
            // 7 setter table
            // 6 __newindex
            // 5 setter table
            ...
            translator.Push(L, type);
            LuaAPI.lua_pushvalue(L, -3);
            // _G[LUA_REGISTRYINDEX]["LuaClassNewIndexs"][type udata] = cclosure cls_indexer
            LuaAPI.lua_rawset(L, -3);
            // pop _G[LUA_REGISTRYINDEX]["LuaClassNewIndexs"]
            LuaAPI.lua_pop(L, 1);
        ...
        /// <param name="L"></param>
        /// <param name="type"></param>
        /// <param name="cls_table">BeginClassRegister调用时是2</param>
        public static void SetCSTable(RealStatePtr L, Type type, int cls_table)
        {
            /*
             * 干了两件事情
             * 1、CS["UnityEngine"]["GameObject"] = class_field_t，class_field_t是一个包含了静态方法和_call方法的table
             * 2、CS[class_field_t udata] = class_field_t，可见用udata是快速获取table的好方法
             */
            ...
            for (int i = 0; i < path.Count - 1; ++i)
            {
                ...
                // 所以这里的作用就是获取table CS["UnityEngine"]，并且把-1pop
                if (0 != LuaAPI.xlua_pgettable(L, -2))
                {
            ...
            // copy class_field_t - 1
            LuaAPI.lua_pushvalue(L, cls_table);
            // CS["UnityEngine"]["GameObject"] = class_field_t
            LuaAPI.lua_rawset(L, -3);
            // pop table CS["UnityEngine"]
            LuaAPI.lua_pop(L, 1);
            // 2：class_field_t
            // 1："UnityEngine.GameObject"
        ...
        /// <summary>
        /// 验证是否是静态函数，并且attribute是MonoPInvokeCallbackAttribute
        /// </summary>
        /// <param name="csFunction"></param>
        /// <returns></returns>
        ...
        /// </summary>
        /// <param name="L"></param>
        public static void PrintStack(RealStatePtr L, Type type1, Type type2, string key = "tsai")
        {
            if (type1 != type2)
            {
                return;
            ...
            if (key != "tsai")
            {
                LuaAPI.PrintStackWithKey(L, key);
            }
            LuaAPI.PrintStackToCS(L);
            //int top = LuaAPI.lua_gettop(L);
            //for (int j = 1; j <= top; j++)
            //{
            //    object t = ObjectTranslatorPool.Instance.Find(L).GetObject(L, j);
            //    if (t == null)
            //    {
            //        Debug.Log("PrintStack：idx = " + j.ToString() + " is nil");
            //        continue;
            //    }
            //    Type tt = t.GetType();
            //    Debug.Log("PrintStack：idx = " + j.ToString() + "  type is " + tt.FullName);
            //    if (tt == typeof(string))
            //    {
            //        Debug.Log("PrintStack：idx = " + j.ToString() + " is string = " + (t as string));
            //    }
            //    if (tt == typeof(LuaTable) && key != null)
            //    {
            //        string value = string.Empty;
            //        ((LuaTable)t).Get(key, out value);
            //        if (!string.IsNullOrEmpty(value))
            //        {
            //            Debug.Log("PrintStack：idx = " + j.ToString() + "key = " + key + " value = " + value);
            //        }
            //    }
            //}
