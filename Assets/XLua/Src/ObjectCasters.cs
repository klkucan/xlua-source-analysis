...
namespace XLua
{
    ...
    /// <summary>
    /// lua对象转cs对象方法集合类
    /// 提供了一堆转换函数，支持LuaTable和LuaFunction
    /// </summary>
    public class ObjectCasters
    {
        ...
        /// </summary>
        /// <param name="L"></param>
        /// <param name="idx">对象在stack上的idx</param>
        /// <param name="target">没用到</param>
        /// <returns></returns>
        private object getLuaTable(RealStatePtr L, int idx, object target)
        {
            // 如果idx上是一个udata,则这个udata就是objects集合的index
            if (LuaAPI.lua_type(L, idx) == LuaTypes.LUA_TUSERDATA)
            {
                object obj = translator.SafeGetCSObj(L, idx);
                return (obj != null && obj is LuaTable) ? obj : null;
            }
            // 不是udata，如果还不是table那就说明这个位置的数据有问题
            ...
            // 将table引用到registry，然后返回ref
            return new LuaTable(LuaAPI.luaL_ref(L), translator.luaEnv);
        }
        /// <summary>
        /// 指定idx转LuaFunction对象，注意luafunction和cfunction在类型上是不一样的
        /// </summary>
        /// <param name="L"></param>
        /// <param name="idx">对象在stack上的idx</param>
        /// <param name="target">没用到</param>
        ...
        private object getLuaFunction(RealStatePtr L, int idx, object target)
        {
            ...
            // 不是udata，如果还不是LuaFunction那就说明这个位置的数据有问题
            ...
            // LuaFunction放到-1
            LuaAPI.lua_pushvalue(L, idx);
            // 将LuaFunction引用到registry，然后返回ref
