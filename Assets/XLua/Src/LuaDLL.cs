...
namespace XLua.LuaDLL
{
    ...
    public partial class Lua
    {
        ...
        [DllImport(LUADLL, CallingConvention = CallingConvention.Cdecl)]
        public static extern int PrintStackToCS(IntPtr L);

        [DllImport(LUADLL, CallingConvention = CallingConvention.Cdecl)]
        public static extern int PrintStackWithKey(IntPtr L, string str);

        /// <summary>
        /// 获取当前栈顶对象，并打印
        /// </summary>
        /// <param name="L"></param>
        public static void print_top(IntPtr L)
        {
            int top = lua_gettop(L);
            Debug.Log("TSAI 栈上对象数：" + top.ToString());
            if (top > 0)
            {
                // LuaAPI.lua_pushstdcallcfunction(rawL, StaticLuaCallbacks.Print);
                xlua_getglobal(L, "print");
                lua_pushvalue(L, -2);
                lua_pcall(L, 1, 0, 0);
            }
        }
