...
namespace XLua.CSObjectWrap
{
    ...
    public class UnityEngineGameObjectWrap
    {
        public static void __Register(RealStatePtr L)
        {
            ...
            // debug用的
            //object t = translator.GetObject(L, 1);
            //Type tt = t.GetType();

            // 这里-1 是字符串 "UnityEngine.GameObject"， 来自init_xlua： local obj = import_type(fqn)
            ...
            // mt构建完成，此时-1还是字符串 "UnityEngine.GameObject"
            ...
            // 构建完成，此时-1还是字符串 "UnityEngine.GameObject"
        }
        ...
        static int _m_GetComponent(RealStatePtr L)
        {
            try
            {
                ...
                // 1位置上是udata
                // FastGetCSObj中获取udata指向的值也就是objects的index，得到真正的cs 对象
                ...
                if (gen_param_count == 2 && translator.Assignable<System.Type>(L, 2))
                {
                    ...
                    // 得到的对象会进入objects，然后在-1留下它的udata
                    translator.Push(L, gen_ret);
